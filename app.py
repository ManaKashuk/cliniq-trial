...
# Heavy libraries are imported lazily only if retrieval indexing is used.
...
def require_login():
    ...
    try:
        _logo = Image.open(ROOT_DIR / "logo.png")  # keep beside this script
...
def next_case_id() -> str:
    counter = int(st.session_state.get("case_counter", 0)) + 1
    st.session_state["case_counter"] = counter
    return f"CLQ-{counter:03d}"


def should_force_refusal(question_text: str) -> bool:
    """Demo safety trigger: risky wording should produce Refuse + Escalate instead of a guessed answer."""
    q = (question_text or "").lower()
    trigger_terms = [
        "mark", "compliant", "late specimen", "late sample", "deviation",
        "ignore", "waive", "dose", "grade", "treat", "steroid",
        "not documented", "without documentation", "temperature excursion",
        "chain of custody", "can i", "is it okay", "should i"
    ]
    return any(term in q for term in trigger_terms)


def refusal_decision_html(question_text: str) -> str:
    safe_q = escape(question_text or "")
    return f"""
    <b>Decision:</b> Refuse + Escalate<br>
    <b>Reason:</b> No approved protocol/SOP source confirms this answer.<br>
    <b>Risk type:</b> Possible protocol deviation / documentation uncertainty<br>
    <b>Escalation target:</b> PI / QA / Regulatory Lead<br>
    <b>Action:</b> Send to human review<br><br>
    <i>CLINIQ does not guess or mark an issue compliant when the approved source is insufficient.</i><br>
    <small>User question: {safe_q}</small>
    """


def make_escalation_record(question_text: str, target: str) -> dict:
    return {
        "Case ID": next_case_id(),
        "Question": question_text,
        "CLINIQ Decision": "Refuse + Escalate",
        "Refusal Reason": "No approved protocol/SOP source confirms this answer.",
        "Risk Type": "Possible protocol deviation / documentation uncertainty",
...
def render_pending_escalation_action() -> None:
    pending = st.session_state.get("pending_escalation")
    if not pending:
        return

    st.markdown("---")
    st.markdown("### 🚦 Refuse + Escalate")
    st.info("CLINIQ refused to answer because the approved SOP/protocol source was insufficient. Route this case to a human reviewer and preserve the audit trail.")

    target = st.selectbox(
        "Escalation target",
        ["PI / QA / Regulatory Lead", "PI / Sub-I", "QA / Compliance", "Regulatory Lead", "Pharmacy / IP", "Medical Monitor", "Sponsor / CRO", "CTU Director"],
        key="pending_escalation_target",
    )

    col1, col2 = st.columns([1, 1])
    with col1:
        if st.button("Escalate for Review", type="primary", use_container_width=True):
            record = make_escalation_record(pending.get("question", ""), target)
            st.session_state.escalation_log.append(record)
            st.session_state.pending_escalation = None
            st.success(f"Escalation case {record['Case ID']} added to the audit trail.")
            rerun()
    with col2:
        if st.button("Clear escalation draft", use_container_width=True):
            st.session_state.pending_escalation = None
            rerun()
...
try:
    favicon = Image.open(ROOT_DIR / "chat.png")  # chat.png must be beside this script
except Exception:
    favicon = "🧭"
...
try:
    logo = Image.open(ROOT_DIR / "logo.png")  # put beside this script
...
# Chat avatar logo
try:
    chat_logo = Image.open(ROOT_DIR / "chat.png")  # put beside this script
...
# Tabs
tab1, tab_audit, tab2 = st.tabs(["💬 Ask CLINIQ", "🚦 Escalation/Audit Trail", "🧪 Benchmark (Expert Scoring)"])
...
with tab1:
            ...
            st.markdown(
                ...
                <div style='text-align:right;margin:10px 0;'>
                    <div style='display:inline-block;background:#e6f7ff;padding:12px;border-radius:12px;max-width:70%;'>
                        <b>You:</b> {escape(str(msg['content']))}
                    </div>
                </div>
                """,
                unsafe_allow_html=True
    ...
    if st.button("Submit", type="primary") and question.strip():
        ...
        if best_match and best_score >= 0.85:
            ...
            if previous_suggestions:
                ...
                # Demo safety behavior: risky or unsupported questions should refuse + escalate, not guess.
                if should_force_refusal(question) or not top_matches:
                    st.session_state.chat_history.append({
                        "role": "assistant",
                        "content": refusal_decision_html(question)
                    })
                    st.session_state.pending_escalation = {
                        "question": question,
                        "reason": "No approved protocol/SOP source confirms this answer.",
                        "risk_type": "Possible protocol deviation / documentation uncertainty",
                    }
                else:
    ...
    render_pending_escalation_action()
...
# =========================
# TAB 2 — Escalation / Audit Trail
# =========================
with tab_audit:
    st.markdown("### 🚦 Escalation / Audit Trail")
    st.caption("Demo workflow layer: when CLINIQ refuses, the case is routed to human review and preserved as an audit-support record.")

    if not st.session_state.escalation_log:
        st.info("No escalation cases yet. In the Ask CLINIQ tab, submit a risky unsupported question such as: 'Can I mark late specimen as compliant?'")
    else:
        audit_df = pd.DataFrame(st.session_state.escalation_log)
        st.dataframe(audit_df, use_container_width=True)
        download_df_as_csv(audit_df, "cliniq_escalation_audit_log.csv", "⬇️ Download Escalation/Audit Log CSV")

        st.markdown("---")
        st.markdown("### CLINIQ Escalation Review")
        case_ids = audit_df["Case ID"].tolist()
        selected_case = st.selectbox("Case ID", case_ids)
        reviewer_decision = st.selectbox(
            "Reviewer decision",
            ["", "Confirmed deviation", "Not a deviation", "Needs PI decision", "Needs QA decision", "Needs sponsor/medical monitor input", "Closed with documentation update"],
        )
        reviewer_notes = st.text_area("Reviewer notes")
        if st.button("Save human review decision", type="primary"):
            for rec in st.session_state.escalation_log:
                if rec["Case ID"] == selected_case:
                    rec["Human Decision"] = reviewer_decision or "Pending"
                    rec["Reviewer Notes"] = reviewer_notes.strip()
                    rec["Status"] = "Reviewed" if reviewer_decision else "Pending Review"
                    rec["Reviewed Timestamp"] = current_ts()
                    break
            st.success("Human review decision saved to audit trail.")
            rerun()

    st.markdown("---")
    with st.expander("Power Automate proof-of-workflow script"):
        st.write("For the demo, this can be described as a lightweight workflow layer:")
        st.code(
            "When CLINIQ Escalation Review form is submitted\n"
            "↓\n"
            "Save response to Excel/SharePoint\n"
            "↓\n"
        ...
        st.caption("This Streamlit table is the local demo version. Power Automate can later route the same case fields to PI/QA/regulatory review.")
