...
def require_login():
    ...
    # --- Header on login page (same look as main app) ---
    try:
        _logo = Image.open(str(ROOT_DIR / "logo.png"))
        _logo_b64 = get_image_base64(_logo)
        st.markdown(
            f"""
            <div style='text-align:left; margin-bottom: 0.5rem;'>
                <img src='data:image/png;base64,{_logo_b64}' width='700'/>
                <h5>SOP Guidance for CAR‑T and High-toxicity Immunotherapy Trial Protocols.</h5>
                <p style='margin-top:0.25rem; color:#444;'>
                    Ask what to do when a study task goes off‑script (missed window, documentation gap, or chain‑of‑custody issue).
                </p>
            </div>
            """,
            unsafe_allow_html=True
    ...
    if st.button("Login", type="primary", key="login_btn"):
        ...
        if (u or "").strip() == allowed_user and (p or "").strip() == allowed_pass:
            ...
            # Store the actual username entered (so it can be displayed after login)
